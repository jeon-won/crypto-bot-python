...
* Date: 2021.05.20.
* Author: Jeon Won
* Func: 가상화폐 급락했을 때(현재가가 볼린저밴드 하한선 밑에 있고 스토캐스틱 %K 값이 낮을 때 급락으로 판단) 텔레그램 메시지 발송
...
while True:
    try: 
        ...
        for ticker in tickers:
            ...
            # 스토캐스틱 %K 값이 낮고 현재가가 볼린저 밴드 하한선 밑에 있고
            if((stoch_k < STOCH_LOWER_LIMIT) and (current_price < bb["lbb"])):
                ...
                # 미보유 중이거나 거래 등록한 코인이 아닌 경우 급락 알림
                if(not balance and not order):
                    message = f"{ticker} Stochastic %K 값: {round(stoch_k, 3)}, Bollinger Bands 하한값: {round(bb['lbb'], 3)}, 현재가: {current_price}원으로 급락" 
