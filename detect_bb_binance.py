...
for ticker in tickers:                 
    ...
    if(bb["per_b"] > 1):
        message = f"binance {ticker} {INTERVAL} 차트 볼린저밴드 %B 값 1 초과 (%B: {round(bb['per_b'], 3)} / 현재가: {bb['current']})"
        bot.sendMessage(TELEGRAM_CHAT_ID, text=message)

    # %B 값 0 미만 시 텔레그램 메시지 전송
    if(bb["per_b"] < 0):
        message = f"binance {ticker} {INTERVAL} 차트 볼린저밴드 %B 값 0 미만 (%B: {round(bb['per_b'], 3)} / 현재가: {bb['current']})"
        bot.sendMessage(TELEGRAM_CHAT_ID, text=message)
